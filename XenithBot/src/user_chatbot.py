...
class UserChatbot():
  ...
  async def initialize_chatbot(self, interaction: discord.Interaction,
                               use_prompt: bool, use_character: bool):
    if self.api_key == None and os.getenv("GOOGLE_API_KEY"):
      self.api_key = os.getenv("GOOGLE_API_KEY")
    elif self.api_key == None:
      await interaction.response.send_message(
          "> **ERROR：Please upload your api key.**", ephemeral=True)
      return False

    genai.configure(api_key=self.api_key)
    self.g_model = genai.GenerativeModel(
    ...
    if self.use_character and (self.avatar_url == None
                               or self.ch_name == None):
      await interaction.response.send_message(
          "> **ERROR：Please use `/character setting` command to set your *avatar_url* and *ch_name* first.**",
          ephemeral=True)
      return False
    ...
    if self.use_prompt:
      if not self.prompt:
        await interaction.response.send_message(
            "> **ERROR：Please use `/character setting` command to set your *prompt* first.**",
            ephemeral=True)
        return False
          ...
          if self.model == "gemini-pro":
            if self.use_character:
              ...
              await send_message(self.chatbot, message, self.thread,
                                 self.avatar_url, self.ch_name, webhook)
            else:
              await send_message(self.chatbot, message, self.thread)
    else:
      await self.thread.send(
          "> **ERROE：Please wait for the previous command to complete.**")
